...
def etl(examples):
    ...
    for (train_image_np, gt_box_np) in examples:
        ...
        train_image_tensors.append(
            tf.expand_dims(tf.convert_to_tensor(train_image_np,
                                                dtype=tf.float32),
                           axis=0))
        gt_box_tensors.append(tf.convert_to_tensor(gt_box_np, dtype=tf.float32))
        zero_indexed_groundtruth_classes = tf.convert_to_tensor(
            [category_index[n] for n in example['label']['class_name']])
...
def get_unique_locations(row, labels_path):
    if not hasattr(get_unique_locations, 'unique_locations'):
        unique_locations = {
            json.load(open(os.path.join(labels_path, f)))['location']
...
def partition_sequential_data(row):
    ...
    location = json.load(open(os.path.join(labels_path,
