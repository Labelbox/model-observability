...
from monitor.settings import PROJECT, DATASET, BBOX_FEATURE_SCHEMA_ID, TEXT_FEATURE_SCHEMA_ID

client = Client()
logger = logging.getLogger("upload")


def create_mal_bbox(datarow_id, t, l, b, r, confidence):
    return {
        "uuid": str(uuid.uuid4()),
...
def upload_annotations(examples):
    ...
    for example in examples:
        ...
        for bbox, confidence in zip(inference["boxes"], inference["scores"]):
            ...
            ndjsons.append(
                create_mal_bbox(example[0].uid, t, l, b, r, round(confidence, 2))
            )

    logger.info("Starting upload..")
    upload_task = PROJECT.upload_annotations(
        name=f"upload-job-{uuid.uuid4()}", annotations=ndjsons, validate=False
    )
...
def sample_training_data(date, target_examples=25):
    ...
    for sample in samples:
        sample = sample["Key"]
        image_res = s3_client.get_object(
            Bucket="images", Key=sample.replace(".json", ".jpg")
        )
